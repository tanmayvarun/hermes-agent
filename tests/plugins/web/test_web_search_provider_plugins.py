...
- All nine bundled plugins (google-search, brave-free, ddgs, searxng,
  exa, parallel, tavily, firecrawl, xai) instantiate and self-report the
...
class TestBundledPluginsRegister:
    ...
    @pytest.mark.parametrize(
        "plugin_name",
        ["google-search", "brave-free", "ddgs", "searxng", "exa", "parallel", "tavily", "firecrawl", "xai"],
    )
...
class TestIsAvailable:
    ...
    def test_google_search_browser_gate_can_be_disabled_and_enabled(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _ensure_plugins_loaded()
        from agent.web_search_registry import get_provider

        p = get_provider("google-search")
        assert p is not None
