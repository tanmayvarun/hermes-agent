...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Plugin & Nous-trained models',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  display: {
    ...
    showAttribution: 'Show a source badge when a turn has a clear upstream provider or tool source.'
  },
  timezone: 'Used when Plugin needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Plugin stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Plugin can read from one file request.',
  approvals: {
    mode: 'How Plugin handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Plugin updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
